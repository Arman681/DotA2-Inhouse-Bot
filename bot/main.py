...
def get_active_store_role_entry(guild_id, user_id, item_key):
    ref = db.collection("store_role_entitlements").document(str(guild_id)) \
            .collection("entries").document(f"{user_id}_{item_key}")
    snap = ref.get()
    if not snap.exists:
        return None
...
async def purchase_store_role(member, item_key, custom_role_name=None, quantity=1):
    ...
    if existing_entry and existing_entry.get("active", False):
        ...
        if expires_at and expires_at.tzinfo is None:
            expires_at = expires_at.replace(tzinfo=timezone.utc)
        base_time = expires_at if expires_at and expires_at > datetime.now(timezone.utc) else datetime.now(timezone.utc)
        ...
        db.collection("store_role_entitlements").document(str(guild.id)) \
          .collection("entries").document(f"{member.id}_{item_key}") \
          .set({
              "role_id": role.id,
              "role_name": role_name,
